...
def ComputeDp(TsContainigSubSeq, indexStartingPosition, window_size, selfJoin, TsToCompare=None):
    ...
    #subSeq A
    subSeq = TsContainigSubSeq[indexStartingPosition:indexStartingPosition + window_size]
    ...
    for i in range(lenTs):
        ...
        if (i + window_size > lenTs):
            break
        elif (i == indexStartingPosition and selfJoin):
            dp.append(np.inf)
            continue
        else:
            ...
            if(len(subSeq) != len(subSeqToCompute)):
                print('----------------------------ERROREEE--------------------------\n len subseq --- len subseqtocompute')
                print(len(subSeq))
                print(len(subSeqToCompute))
...
#RIGHE COMMENTATE = CODICE ORIGINALE
#MODIFICO DIRETTAMENTE QUESTO CODICE COSI EFFETTUO MENO MODIFICHE IN GENERALE ( ALTRIMENTI DOVREI MOFICIARE TUTTE LA GERARCHIA DI CHIAMATE stomp->distProfile->...  )
def naiveDistanceProfile(tsA,idx,m,tsB = None):
    ...
    distanceProfile=ComputeDp(tsA,idx,m,equal_arrays,tsB) #invoco la mia funzione per calcolare DP
